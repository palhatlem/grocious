...
own machine. No cloud, no account with anyone but the chains you already shop at.
...
| Endpoint | Returns |
|---|---|
| `GET /api/summary` | Dashboard data per chain: balances, yearly earnings, discounts, counts |
| `GET /api/export/<YYYY-MM>.json` | Every receipt for a month; add `?lines=1` for item lines |
...
- [ ] Inbox — share, photograph or e-mail in receipts from everywhere else, into the same archive
